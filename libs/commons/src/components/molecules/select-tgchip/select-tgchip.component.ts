import {
  Component,
  EventEmitter,
  Input,
  Output,
  OnInit,
  OnChanges,
} from '@angular/core';
import { Options } from '../../../models/options';

@Component({
  selector: 'claro-select-tgchip',
  templateUrl: './select-tgchip.component.html',
  styleUrls: ['./select-tgchip.component.scss'],
})
export class SelectTgChipComponent implements OnInit, OnChanges {
  @Input() value: any = '';
  @Input() disabled = false;
  @Input() placeholder = 'Selecciona una opción';
  @Input() options: Options[] = [];
  @Input() size = 'md';
  @Input() isObject = false;
  @Input() autoSelect = true;
  @Output() public valueChanged = new EventEmitter();
  @Output() public elementBlurred = new EventEmitter();
  classes: string[];

  ngOnInit() {
    this.classes = ['w-100', 'size-' + this.size];
  }

  ngOnChanges() {
     console.log("SelectTgChipComponent: " + this.disabled)
    if(!this.value && this.disabled) {
      this.disabled = true;
    } else {
      if (!this.value){
        if(this.options?.length > 1){
          this.disabled = false;
        }
        if(this.autoSelect && this.options?.length === 1) {
          setTimeout(() => {
            this.value = this.isObject ? this.options[0] : this.options[0].value;
            this.valueChanged.emit(this.value);
            this.disabled = true;
          });
        }
      }
    }
  }

  onChange(ev) {
    this.valueChanged.emit(ev.value);
  }

  onTouched() {
    this.elementBlurred.emit();
  }
}
